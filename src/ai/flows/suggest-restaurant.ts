...
const SuggestRestaurantInputSchema = z.object({
  ...
  priceRange: z.enum(['$', '$$', '$$$']).optional().describe('Desired price range for restaurants.'),
  distanceKm: z.number().min(1).max(30).optional().describe('Search radius in kilometers from the location.'),
...
{{#if priceRange}}
Focus on restaurants roughly in the {{{priceRange}}} price range.
{{/if}}

{{#if distanceKm}}
Look for options within {{{distanceKm}}} kilometers of the specified location.
{{/if}}

{{#if cuisine}}
Focus on the following cuisines: {{#each cuisine}}{{{this}}}{{#unless @last}}, {{/unless}}{{/each}}.
{{/if}}
